...
/**
	Altered to utilize lodash in favor of native methods.
	Hoping to get performance gains
...
EPUBJS.core.extend = function(target) {
    ...
    _.each(sources, function (source) {
      if(!source) return;
      _.each(Object.getOwnPropertyNames(source), function(propName) {
	...
	hooks.prototype.getHooks = function(){
		...
		for (var plugType in this.hooks) {
			...
			_.each(plugs, function(hook){
				self.registerHook(plugType, hook);
			});
		}
	};
		...
		if(typeof(this.hooks[type]) != "undefined"){
			...
			if(typeof(toAdd) === "function"){
				...
				_.each(toAdd, function(hook){
					if(toFront) {
						self.hooks[type].unshift(hook);
					}else{
						self.hooks[type].push(hook);
					}
				});
			}
		}else{
		...
		if(typeof(this.hooks[type]) != "undefined"){
			...
			if(typeof(toRemove) === "function"){
				...
				_.each(toRemove, function(hook){
					index = self.hooks[type].indexOf(hook);
					if (index > -1) {
						self.hooks[type].splice(index, 1);
					}
				});
			}
		}
...
EPUBJS.Parser.prototype.navItems = function (navNode, spineIndexByURL, bookSpine) {
	...
	_.each(items, function (item) {
		...
		var content = item.querySelector('a, span'),
				...
				subitems = self.navItems(item, spineIndexByURL, bookSpine);
...
EPUBJS.Renderer.prototype.addEventListeners = function(){
	...
	_.each(this.listenedEvents, function(eventName){
		self.render.document.addEventListener(eventName, self.triggerEvent.bind(self), false);
	});
...
EPUBJS.Renderer.prototype.removeEventListeners = function(){
	...
	var self = this;
	_.each(this.listenedEvents, function(eventName){
		self.render.document.removeEventListener(eventName, self.triggerEvent, false);
	});
